...
import { isValidName, isValidPositiveDigits, sanitizeName, sanitizePositiveDigits } from '../../utils/validation';
...
export function ManualBookingPage() {
  ...
  const isZeroPriceTrip = !!(
    selectedTrip &&
    Number(selectedTrip.price ?? 0) <= 0 &&
    (!selectedTrip.offer_is || Number(selectedTrip.price_offer ?? 0) <= 0)
  );
        ...
        <Card>
          ...
          <div className="mt-4 space-y-3">
            <Field label={messages.company.manualBooking.trip}>
              ...
              {isZeroPriceTrip ? (
                <div className="mt-2 rounded-xl border border-amber-200 bg-amber-50 p-2.5 text-xs font-medium text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
                  ⚠️ تنبيه: سعر هذه الرحلة محدد بـ 0 ل.س. يرجى الانتقال إلى قائمة "الرحلات" وتعديل سعر الرحلة ليكون أكبر من الصفر قبل إجراء الحجز.
                </div>
              ) : null}
            </Field>
                    ...
                    onChange={(e) =>
                      setPassengers(
                        passengers.map((item, itemIndex) =>
                          itemIndex === index ? { ...item, full_name: sanitizeName(e.target.value) } : item,
                        ),
                      )
                    }
                    placeholder="نص فقط"
                ...
                <Field label={messages.common.phone}>
                  <Input
                    ...
                    onChange={(e) =>
                      setPassengers(
                        passengers.map((item, itemIndex) =>
                          itemIndex === index ? { ...item, phone: sanitizePositiveDigits(e.target.value) } : item,
                        ),
                      )
                    }
                    placeholder="أرقام موجبة فقط"
                  />
                </Field>
                <Field label={messages.company.manualBooking.nationalId}>
                  <Input
                    ...
                    onChange={(e) =>
                      setPassengers(
                        passengers.map((item, itemIndex) =>
                          itemIndex === index ? { ...item, national_id: sanitizePositiveDigits(e.target.value) } : item,
                        ),
                      )
                    }
                    placeholder="أرقام موجبة فقط"
