...
export async function getCompanyKpis(companyId?: string | null) {
  ...
  const tripIds = (companyTripsRes.data ?? []).map((t) => t.id);

  let bookings: any[] = [];
  if (tripIds.length > 0) {
    // Chunk requests to avoid URL/query parameter length limits
    const chunkSize = 200;
    const promises = [];
    for (let i = 0; i < tripIds.length; i += chunkSize) {
      ...
      promises.push(
        supabase
          .from('bookings')
          .select('count_passengers,price_total,rating_value')
          .in('trip_id', chunk)
          .limit(10000),
      );
    }
  ...
  if (tripIds.length > 0) {
    ...
    for (let i = 0; i < tripIds.length; i += chunkSize) {
      ...
      promises.push(
        supabase
          .from('bookings')
          .select('id, trip_id, count_passengers, price_total, created_at, payments(payment_method)')
          .in('trip_id', chunk)
          .limit(10000),
      );
    }
    ...
    for (const res of chunkResults) {
      if (res.data) {
        allBookings.push(...(res.data as any));
      }
    }
  }
